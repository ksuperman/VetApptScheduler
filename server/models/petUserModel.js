 ...
 */
const TABLE_NAME = 'PUBLIC.PET_USER';
...
const SQL_STATEMENT_BY_OPERATION = {
    INSERT_PET_USER: `INSERT INTO ${TABLE_NAME} (${tableColumnNames.slice(1).join(',')}) VALUES (${tableColumnNames.slice(1).map((column, index) => `$${index + 1}`).join(',')}) RETURNING *`,
    FIND_PETS_BY_USER_ID: `SELECT PET_ID FROM ${TABLE_NAME} WHERE USER_ID = $1`,
};
